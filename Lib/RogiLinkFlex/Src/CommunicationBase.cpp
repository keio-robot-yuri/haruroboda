...
uint8_t* _global_header_removed_buffer_ptr; // なぜかglobalにしないと怒られる // TODO: 原因調査
...
void CommunicationBase::set_device_id(uint8_t device_id) {
    this->device_id = device_id;
}
...
void CommunicationBase::add_timeout_callback(uint8_t frame_id, uint32_t timeout, std::function<void()> callback) {
    if (timeout_callback_map.find(frame_id) == timeout_callback_map.end()) {
        timeout_callback_map[frame_id] = std::vector<std::tuple<uint32_t, std::function<void()>>>();
        last_receive_time_map[frame_id] = HAL_GetTick();
    }
    timeout_callback_map[frame_id].push_back(std::make_tuple(timeout, callback));
}
...
void CobsEncodedCommunicationBase::on_receive_raw(uint8_t* data, uint8_t size) {
    ...
    // 受信時刻を更新
    if (timeout_callback_map.find(id) != timeout_callback_map.end()) {
        for (auto &timeout_callback : timeout_callback_map[id]) {
            if (HAL_GetTick() - last_receive_time_map[id] > std::get<0>(timeout_callback)) {
                std::get<1>(timeout_callback)();
            }
        }
    }
}
